...
interface DashboardCommonProps {
  summaryData: {
    elderName: string;
    healthScore: number;
    nextCall: { date: string; time: string; dayName: string };
  ...
  children: React.ReactNode; // 통화 기록 렌더링 부분
  callListCount?: number; // 통화 기록 개수 (기본값: 5)
  showQuickActions?: boolean; // 빠른 액션 표시 여부 (기본값: true)
  quickActionsPosition?: 'sidebar' | 'bottom'; // 빠른 액션 위치
}
...
export function DashboardCommon({
  ...
  quickActionsPosition = 'sidebar'
}: DashboardCommonProps) {
  const router = useRouter();

  return (
    <div>
      {/* 메인 그리드 - 최근 통화 기록 + 다음 통화 일정 */}
      <div className="grid grid-cols-12 gap-6 mb-6">
          ...
          <div className="bg-white rounded-md shadow-sm border border-slate-200 p-6 h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-violet-600 hover:text-violet-700 font-bold text-sm transition-colors"
              >
                <span>전체 기록 보기</span>
        ...
        {/* 우측: 다음 통화 일정 + 이번 주 통화 일정 + Quick Actions */}
        <div className="col-span-12 lg:col-span-4 flex flex-col space-y-6">
          ...
          <div className="bg-white rounded-md shadow-sm border border-slate-200 p-6 flex-1 flex flex-col">
            ...
            <div className="space-y-3 flex-1">
              {upcomingCalls.map((call) => (
                <div key={call.id} className="flex items-center justify-between p-4 rounded-md bg-slate-50 border border-slate-200 hover:bg-violet-50 hover:border-violet-200 transition-all">
                  <div className="flex items-center gap-3">
          ...
          {/* Quick Actions (긴급 알림, 주간 리포트) - 우측 사이드바에만 표시 */}
          {showQuickActions && quickActionsPosition === 'sidebar' && (
            <div className="bg-white rounded-md shadow-sm border border-slate-200 p-6">
      ...
      {/* Quick Actions - 하단에 표시 (밑줄 갭이 클 때) */}
      {showQuickActions && quickActionsPosition === 'bottom' && (
        <div className="mt-6 bg-white rounded-md shadow-sm border border-slate-200 p-6">
          <h3 className="text-sm font-black text-slate-900 mb-1">빠른 액션</h3>
          <QuickActions showSubtitle={true} />
        </div>
      )}
